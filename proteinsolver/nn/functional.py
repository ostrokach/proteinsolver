...
def sparse_multi_head_attention_forward(
    ...
    attn_output_weights = sparse_softmax(attn_output_weights.transpose(0, 1), index[0]).transpose(
        0, 1
    )
    attn_output_weights = dropout(attn_output_weights, p=dropout_p, training=training)

    attn_output = scatter_("add", attn_output_weights.unsqueeze(-1) * v, index[0], dim=1)
